...
class ArtDecoBorder {
    ...
    buildRect(group: d3.Selection<SVGGElement, unknown, HTMLElement, any>) {
        ...
        [delta * 2, delta, 0].forEach( function(d,i) {
            ...
            const lineRect: string = d3.line()([[d, d], [d + sizeLargeRect, d], [d + sizeLargeRect, d + sizeSmallRect],
            [d + sizeSmallRect, d + sizeSmallRect],
            [d + sizeSmallRect, d + sizeLargeRect], [d, d + sizeLargeRect], [d, d]]) as string
    ...
    buildArc(group: d3.Selection<SVGGElement, unknown, HTMLElement, any>) {
        const thisBorder = this;
        group.append("path")
            .attr("d", d3.arc()
    ...
    createAnimation() {
        const thisBorder = this;
        let currentClassNumber : number =  2;
