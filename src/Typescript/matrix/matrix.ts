    ...
    function matrix_diag(c : number, n : number) {
        var i, j, Z = Array(n*n);
    ...
    function matrix_transpose(X : Array<number>, n : number, m : number) {
        var i, j, Z = Array(m*n);
    ...
    function matrix_scale(X : Array<number>, c : number, n : number, m : number) {
		var i, j;
    ...
    function matrix_add(X : Array<number>, Y : Array<number>, n : number, m : number) {
		var i, j, Z = Array(n*m);
		for(i=0;i<n;i++)
			for(j=0;j<m;j++)
	...
	function matrix_multiply(X : Array<number>, Y : Array<number>, n : number, m : number, p : number) {
	...
	function matrix_chol (X : Array<number>, n : number) { 
	var i, j, k, p = Array(n);
    ...
    function matrix_chol2inv(X : Array<number>, n : number) {
		var i, j, k, sum;
		for(i=0;i<n;i++) {
			X[i*n+i] = 1/X[i*n+i];
			for(j=i+1;j<n;j++) {
	...
	function matrix_solve(X : Array<number>, n : number) {
	var m = n;
	var b = Array(n*n);
	var indxc = Array(n);
	var indxr = Array(n);
	var ipiv = Array(n);
	var i, icol : number, irow : number, j, k, l, ll;
